...
//! The rasterizer/presenter is selected at compile time: CPU `wl_shm` +
//! `egui_software_backend` (`cpu` feature) or `wgpu` (`gpu` feature).
//!
...
use smithay_client_toolkit::{
    ...
    seat::{
        ...
        pointer::{
            PointerEvent, PointerEventKind, PointerHandler, BTN_LEFT, BTN_MIDDLE, BTN_RIGHT,
        },
...
pub fn run(ctx: egui::Context, app: Hring) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // which on a CPU rasterizer is a large win. `wp_viewport` is required to
    ...
    #[cfg(feature = "cpu")]
    let presenter = Presenter::new(&shm).expect("failed to create CPU presenter");
    #[cfg(feature = "gpu")]
    let presenter = Presenter::new(&conn, layer.wl_surface())
        .expect("failed to create GPU presenter (build with `--no-default-features --features cpu` for the software backend)");
...
impl SeatHandler for Backend {
    ...
    fn remove_capability(
        ...
        if capability == Capability::Keyboard && let Some(keyboard) = self.keyboard.take() {
            keyboard.release();
        }
...
#[cfg(feature = "cpu")]
smithay_client_toolkit::delegate_shm!(Backend);
